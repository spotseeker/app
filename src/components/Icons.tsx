...
const ImageIcon2 = ({ size = 60 }: Props) => {
  return <MaterialCommunityIcons name="image-outline" size={size} color="#fb9062" />
}
...
const NotificationsIcon = ({ size = 25 }: Props) => {
  return <MaterialCommunityIcons name="bell-outline" size={size} color="#6a0d83" />
}
...
const ArrowIcon = ({ shown, size = 25 }: Props) => {
  return <AntDesign name={!shown ? 'right' : 'down'} size={size} color={themeIconColor} />
}
...
const LogoBigIcon = ({ width = 241, height = 399, mr }: pngProps) => {
  return <Image source={logoBig} style={{ width: width, height: height, margin: mr }} />
}
